...
def fetch_news_via_claude(api_key: str, tickers: list[str]) -> str:
    """Use Claude with web search to fetch latest sustainability stock news.
    Uses requests library directly to avoid SDK header encoding issues.
    """
    ...
    try:
        ...
        return "News unavailable — analysis will be based on price data only."
...
def main():
    ...
    # 2. Fetch news
    news = fetch_news_via_claude(api_key, tickers)
